...
const NAV_CARDS = [
  { page: 'mycourse',     name: '我的课程', desc: '在此管理你的课程' },
  { page: 'allcourse',    name: '课程广场', desc: '在此浏览全校课程' },
  { page: 'explore',      name: '发现',     desc: '点击开始发现' },
  { page: 'treasurebox',  name: '百宝箱',   desc: '会有你需要的' },
  { page: 'profile',      name: '个人中心', desc: '此处管理个人资料' },
  { page: 'notifications',name: '通知中心', desc: '查看消息与提醒' },
  { page: 'my_post',      name: '我的发布', desc: '管理你发布的帖子' },
...
function animateHero(container) {
  const textEls = Array.from(container.querySelectorAll('.home-hero-text > *'));
  // Slow, gentle reveal: brand → h1 → actions → footer
  // Each line waits 200ms after the previous, each takes 1000ms to fade up
  animStagger(textEls, { y: 36, dur: 1000, gap: 200, ease: Ease.gentle });

  // Image fades in after text has begun revealing
  const imageEl = container.querySelector('.home-hero-image');
  if (imageEl) {
    animIn(imageEl, { y: 0, dur: 1200, delay: 600, ease: Ease.gentle });
  }
}
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const cards = entry.target.querySelectorAll('.home-stat-card, .home-nav-card');
        if (cards.length) {
          animStagger(Array.from(cards), { y: 28, dur: 800, gap: 100, ease: Ease.gentle });
        }
        // Also animate the label and title
        const headEls = entry.target.querySelectorAll('.home-section-title');
        if (headEls.length) {
          animStagger(Array.from(headEls), { y: 22, dur: 750, gap: 140, ease: Ease.gentle });
        }
...
function renderHero() {
  const footerText = isLoggedIn()
  return `
    <section class="home-hero">
      <div class="home-hero-text">
        <div class="home-hero-brand">
          <img src="/images/blue-whale-logo.svg" alt="" aria-hidden="true">
          <span>课搭子</span>
        </div>
...
function renderStatCard(icon, value, label, targetPage) {
  ...
  return `
    <section class="home-section">
      ...
      <div class="home-stats" id="home-stat-grid">
        ${renderStatCard(PLUGIN_ICON.calendar, todayCount || courses.length, '今日课程', 'mycourse')}
        ${renderStatCard(PLUGIN_ICON.check, `${streak}天`, '连续签到', 'profile')}
...
function renderNavCards() {
  ...
  return `
    <section class="home-section">
      <h2 class="home-section-title">探索更多</h2>
      <div class="home-nav-grid">${cards}</div>
    </section>
  `;
}
...
registerPage('home', async (container) => {
  ...
  // 1. Animate hero text with staggered reveal
  animateHero(container);

  // 2. Setup scroll-triggered section reveals
  observeSections(container);

  // 3. Load data and update stats
